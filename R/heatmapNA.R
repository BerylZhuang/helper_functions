...
## downloaded 2015-12-17
## modified so that the missing value (NA) are colored grey(default) instead of white
## change the main function to pheatmapNA
# 
#  USAGE
#  pheatmapNA(mat,colour_scheme = "YlOrRd",NA_color = "grey",bk_length = 255, ...)
#  
#  colour scheme is is the ColorBrewer palettes e.g. BuPu GnBu Greens, default is YlOrRd
#  NA_color: color for missing values, default is grey
#  bk_length: # of color breaks
#  ... all other parameters in pheatmap()
#  
library(gtable)
library(scales)
library(grid)
...
lo = function(rown, coln, nrow, ncol, cellheight = NA, cellwidth = NA, treeheight_col, treeheight_row, legend, annotation_row, annotation_col, annotation_colors, annotation_legend, annotation_names_row, annotation_names_col, main, fontsize, fontsize_row, fontsize_col, gaps_row, gaps_col, ...){
    ...
    # Produce gtable
    gt = gtable::gtable(widths = unit.c(treeheight_row, annot_row_width, mat_width, rown_width, legend_width, annot_legend_width), heights = unit.c(main_height, treeheight_col, annot_col_height, mat_height, coln_height), vp = viewport(gp = do.call(gpar, gp)))
...
draw_legend = function(color, breaks, legend, NA_include, ...){
    height = min(unit(1, "npc"), unit(150, "bigpts"))
    if(NA_include){ ## the last break element is the break for NA value
        breaks <- breaks[1: length(breaks)-1]
    }
    
    legend_pos = (legend - min(breaks)) / (max(breaks) - min(breaks))
    legend_pos2 = (legend - min(breaks)) / (max(breaks) - min(breaks))
    legend_pos = height * legend_pos + (unit(1, "npc") - height)
    
    breaks = (breaks - min(breaks)) / (max(breaks) - min(breaks))
    breaks2 = (breaks - min(breaks)) / (max(breaks) - min(breaks))
...
heatmap_motor = function(matrix, border_color, cellwidth, cellheight, tree_col, tree_row, treeheight_col, treeheight_row, filename, width, height, breaks, color, legend, annotation_row, annotation_col, annotation_colors, annotation_legend, annotation_names_row, annotation_names_col, main, fontsize, fontsize_row, fontsize_col, fmat, fontsize_number, number_color, gaps_col, gaps_row, labels_row, labels_col, 
    ...
    # Draw legend
    if(!is.na2(legend)){
        elem = draw_legend(color, breaks, legend, NA_include, fontsize = fontsize, ...)
...
generate_annotation_colours = function(annotation, annotation_colors, drop){
    ...
    factor_colors = scales::dscale(factor(1:count), scales::hue_pal(l = 75))
...
#' heatmaps2()
#' 
#' The pheatmap() function in pheatmap package. A function to draw clustered heatmaps where one has better control over some graphical 
#' parameters such as cell size, etc. Slightly modified to incorporate colour for NA. Use \code{"heatmapNA()"}
#' 
...
#' @param NA_include boolean specifying if NA value is predifined as a dummy non NA value. must use with
#' color and breaks, where the last element is the defined color of NA and defined break value of the NA (a dummy value, eg 5000)
#'  Default is F.
...
pheatmap2 = function(mat,NA_include = F,
    ...
    if(is.na2(breaks)){ ## check if breaks is a list
        breaks = generate_breaks(as.vector(mat), length(color))
    }
    
    if(NA_include){         ## this step to add the color bar label (doesn't change the range of the legend)
        breaks2 <- breaks[1: length(breaks)-1]  ## remove the last element which is the NA dummy value
        if (legend & is.na2(legend_breaks)) {
            legend = grid.pretty(range(as.vector(breaks2)))
            names(legend) = legend
        } else if(legend & !is.na2(legend_breaks)){
            legend = legend_breaks[legend_breaks >= min(breaks2) & legend_breaks <= max(breaks2)]
            if(!is.na2(legend_labels)){
                legend_labels = legend_labels[legend_breaks >= min(breaks2) & legend_breaks <= max(breaks2)]
...
#' pheatmapNA()
#' 
#' Heatmap function based on pheatmap. NA colours can be defined
#' 
#' @param mat numeric matrix of the values to be plotted.
#' @param NA_color for missing/NA values, default = 'grey'
#' @param colour_scheme the ColorBrewer palettes e.g. 'BuPu' 'GnBu' 'Greens', default = 'YlOrRd'
#' @param bk_length: # of color breaks, default = 255
#' @param ... all other parameters in pheatmap()
#' @param kmeans_k the number of kmeans clusters to make, if we want to agggregate the 
#' rows before drawing heatmap. If NA then the rows are not aggregated.
#' @param breaks a sequence of numbers that covers the range of values in mat and is one 
#' element longer than color vector. Used for mapping values to colors. Useful, if needed 
...
pheatmapNA <- function(mat,colour_scheme = "YlOrRd",
                       NA_color = "grey",bk_length = 255, ...){
    library("RColorBrewer")
    ## NA_color: define color for NA value
    na_index <- which(is.na(mat))
    # replace NA 
    if(length(na_index) >0){
        (new_range <- range(na.omit(as.numeric(mat))))
        (tmp_value <- ceiling(max(new_range))*10)  ## define a dummy NA value that's >> max value
        (bk = unique(c(seq(new_range[1], new_range[2], length=bk_length), tmp_value))) 
        ## replace NA cells with dummy NA value
        mat[na_index] <- tmp_value
        
        ## define color for NA and other values, note that length(cols) = length(bk)-1
        (palette <- colorRampPalette(rev(brewer.pal(n = 9, colour_scheme))))
        (paletteSize <- length(bk)-2)
        (cols <- c(palette(paletteSize),NA_color))
        pheatmap2(mat, NA_include =T, 
                  color = cols, breaks=bk,...)  
    }else{
        palette <- colorRampPalette(rev(brewer.pal(n = 9, colour_scheme)))
        paletteSize <- bk_length +1
        cols <- palette(paletteSize)
        pheatmap2(mat, NA_include =F, 
                  color = cols,...)  
    }
}
